...
| Key | Description |
|---|---|
| Ctrl+C or Ctrl+I | Inventory: Equipment, one tab per bag, Stats. On an equipment slot Enter opens a picker of everything that fits, Backspace unequips. In a bag Enter is the game's right-click (equip / drink / read); on a component it opens a picker of the items it can be attached to. In Stats, Enter on Physique / Cunning / Spirit spends a point. **Bags:** a pickup joins an existing stack in any bag, else goes into bag 1, else into the one bag marked "receives pickups" -- nothing else, so a full bag 1 reads "inventory full" until you mark another. **Ctrl+Enter on a bag tab marks it** (the tab the cursor is on, or the open one); that bag's page then starts with "receives pickups". Marking bag 1 just restores the default |
