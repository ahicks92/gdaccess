...
| Ctrl+\ | Sound cue settings. First Tab stop: one on/off row per positioned cue -- wall tones, harmful ground nearby, standing in harmful ground, way out of harmful ground, and the sonar's enemy, loot, entrance, breakable, shrine and interactable pings (Enter flips). Second Tab stop: four volumes in percent -- wall tones, harmful ground, enemy pings, other pings (Left/Right by 10; Enter steps up and wraps). Escape closes; everything is saved between sessions. The bare backslash still switches the whole sonar off and on |
