...
| Ctrl+\ | Sound cue settings. First Tab stop: one on/off row per positioned cue -- wall tones, harmful ground nearby, standing in harmful ground, way out of harmful ground, and the sonar's enemy, loot, entrance, breakable, shrine and interactable pings (Enter flips). Second Tab stop: six volumes in percent -- wall tones, harmful ground, enemy pings, other pings, the enemy voice (Mark) and your voice (Zira) (Left/Right by 10; Enter steps up and wraps). Escape closes; everything is saved between sessions. The bare backslash still switches the whole sonar off and on |
...
| Ctrl+K, Ctrl+G, Ctrl+H, Ctrl+V, Ctrl+B, Ctrl+X, Ctrl+Z, Ctrl+P, Ctrl+], Ctrl+Enter | The game's own group, game menu, help, achievements, drop item, item tooltips, show items, pause, toggle UI, chat (the game's party display, a multiplayer HUD element, is not reachable: Ctrl+\ is the sound cue settings) |
