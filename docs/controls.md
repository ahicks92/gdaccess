...
|---|---|---|
| Inventory (C or I) | Equipment, one per bag, Stats | Equipment: "slot, item"; Enter unequips into the bag, Space = the game's tooltip. On a **weapon slot** (Right/Left Hand), Ctrl+J / Ctrl+I put the weapon's **basic attack** back on the left / right mouse (Ctrl+1..0 to a quickbar slot) -- the way to recover a basic attack after re-slotting the mouse; the id comes from the game's own default for the equipped weapon. Bag: "item, x N" in reading order; Enter = the game's right-click (equip / drink / read), Space = tooltip. Stats: the character sheet; on Physique / Cunning / Spirit, Enter spends an attribute point |
