...
| Ctrl+M | The map: opens the game's aerial map and presents it accessibly as a two-tab picker -- **quest markers** and **points of interest** (merchants, riftgate, spirit guide, NPCs, ...). Each icon is the game's own map marker, named by matching it to the entity under it, listed nearest-first with distance and clock bearing. Tab moves from the tab row into the list; Enter picks a marker as the follow target ("following \<name\>") and closes the map. The marker set is whatever the game currently draws on the map (the loaded area) -- see docs on scope |
