...
| Shift (with J / I / a number key) | The game's Hold Position key, passed through (2026-09-23): the exe's key action sets its stationary byte (X+0x848, exe+0x27020 on down, exe+0x26c41 on up) and `HandleActionFromMouse` then stops movement and acts in place. J / I used to require no modifier at all; they now ignore Shift (Ctrl / Alt still block them). Matters most with the game's Classic Casting option, where a plain press walks into range. Not verified live |
| (J / I / number keys) | Direct aim (2026-09-23): the locked target is handed to the game itself -- `HotSlotOptionSkill::Activate` gets it in the controller's combat enemy + mouse repeat data, `HandleActionFromMouse` gets its id and position (`world::direct_aim_apply`) -- so an off-screen target is hit, not just aimed at along the line; the game's own range / walk / clamp rules still apply. Always on (an F12 A/B toggle existed for one day) |
...
| ; (semicolon) | Ping the reviewed thing again: one of three sounds, checked in this order -- unreachable (the game's pathfinder does not arrive there), straight line (your character has line of sight and the walk is barely longer than the straight line), path around (reachable, but round something); `world::route_kind`, 2026-09-23 -- panned toward it, fading with distance; also played on every landing, and re-sounded automatically whenever the route kind changes while a thing is under review (walk into line of it, an obstacle clears, it goes unreachable) -- no key needed |
...
| ' (apostrophe) | Follow the picked map marker: plays the route ping toward it (unreachable / straight line / path around, panned, fading with distance, like ;) and speaks "\<name\>, N away, H o'clock". Tracks a moving NPC; "not following anything" until you pick one from Ctrl+M |
