...
//! installed per user under %LOCALAPPDATA%\Programs\GD Access; its log and settings stay in
...
pub const UNINSTALL_KEY: &str = "GDAccess";

fn local_app_data() -> PathBuf {
    dirs::data_local_dir().unwrap_or_else(|| PathBuf::from("C:\\Users\\Default\\AppData\\Local"))
}

/// %LOCALAPPDATA%\Programs\GD Access -- the per-user programs folder (no admin, survives anything Steam does).
...
mod tests {
    ...
    fn install_dir_is_under_programs() {
        assert!(install_dir().ends_with("Programs\\GD Access"));
