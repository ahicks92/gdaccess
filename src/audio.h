...
void play_sample(const std::string& wav_path, float volume, float pan, float rear_shelf_db = 0.0f, bool apply_master = true, int group = 0, bool replace_group = false,
                 float semitones = 0.0f, float predelay_ms = 0.0f);   // apply_master=false: like the voices, above the 0.6 master
// semitones != 0 plays a resampled copy (pitch and length change together, like a tape; cached per file + shift),
// predelay_ms starts it that much later (the route ping's north/south echo, the sonar's in-range pitch).
