...
// announcement per decade crossed, in either direction, no jitter inside a decade. The spoken number is the
// ACTUAL percentage at the crossing (100 -> 99 says 99, not the bucket floor 90). Engine-free, unit-tested.
namespace gd::core {
class ThresholdWatcher {
 public:
  explicit ThresholdWatcher(double step = 0.10) : step_(step > 0 ? step : 0.10) {}
  void reset() { bucket_ = -1; }
  // true -> announce out_percent (the current fraction as a rounded percentage). The first update after a reset
