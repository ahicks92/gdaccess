...
// enemy ping, and the sonar's other pings. Persisted through settings (key=value, %LOCALAPPDATA%\gdaccess\settings.txt)
...
enum Channel { Walls = 0, Hazards, EnemyChannel, Other, kChannels };
...
float gain(Channel ch);             // volume / 100 as a linear factor
}  // namespace gd::cues
