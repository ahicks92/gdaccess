...
static std::string handle(const std::string& path, const std::map<std::string, std::string>& q, const std::string& body, int& status) {
  ...
  if (path == "/sonar") {   // the sonar field: ?on=0|1 &radius= &vol= &ref= &floor= &pnear= &pfar= &dnear= &dfar= &force= &cap= &ratio= &pivot= &slew= &window= &hash= &grace=
    if (q.count("on")) sonar::set_enabled(truthy(q.at("on")));
    if (q.count("compress")) sonar::set_compression_enabled(truthy(q.at("compress")));
    for (const char* k : {"radius", "vol", "ref", "floor", "pnear", "pfar", "dnear", "dfar", "force", "cap", "ratio", "pivot", "slew", "window", "hash", "grace"}) if (q.count(k)) sonar::set_knob(k, (float)atof(q.at(k).c_str()));
