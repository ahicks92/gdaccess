...
| prism speech SDK | `prism-bin/prism-sdk-v0.18.1/` -- only the parts the build uses are committed: `include/`, `windows/x64/dynamic/release/{lib/prism.lib,bin/prism.dll}`, `LICENSES/`, `NOTICE` (the full prebuilt release is 1.1 GB and stays ignored by `.gitignore`) | prebuilt release v0.18.1 from the prism GitHub releases; the build delay-loads `prism.dll` from next to `grimdark.dll` | MPL-2.0 (see its `NOTICE` and `LICENSES/`) |
